...
// Example: each symbol is std::vector<std::complex<float>>
// rx_pilots.get_symbol(i_dmrs, i_layer)
// pilots.get_symbol(hop_offset + i_dmrs, i_layer)
...
void init_udp()
{
    ...
    python_addr.sin_port = htons(5005);
    python_addr.sin_addr.s_addr = inet_addr("127.0.0.1");
}

void send_symbol_udp(const srsran::span<const std::complex<float>>& rx,
                     const srsran::span<const std::complex<float>>& tx,const srsran::span<const std::complex <float>>& est_lse_sym)
{
    if (udp_socket < 0) {
        udp_socket = socket(AF_INET, SOCK_DGRAM, 0);
    ...
    for (int i = 0; i < N; i++) {
        //buffer.push_back(symbol_index);
        //buffer.push_back(last_symbol);
        buffer.push_back(rx[i].real());
        buffer.push_back(rx[i].imag());
        buffer.push_back(tx[i].real());
        buffer.push_back(tx[i].imag());
        buffer.push_back(est_lse_sym[i].real());
        buffer.push_back(est_lse_sym[i].imag());
                                                               ...
                                                               unsigned i_layer)
{
  ...
  srsvec::prod_conj(rx_pilots.get_symbol(0, i_layer), pilots.get_symbol(hop_offset, i_layer), pilots_lse);
    auto rx_sym = rx_pilots.get_symbol(0, i_layer);
    auto tx_sym = pilots.get_symbol(hop_offset, i_layer);
    auto est_lse_sym=pilots_lse;

    send_symbol_udp(rx_sym, tx_sym,est_lse_sym);
  ...
  rx_sym = rx_pilots.get_symbol(1, i_layer);
  tx_sym = pilots.get_symbol(hop_offset + 1, i_layer);
  est_lse_sym=pilot_products;
  ...
  if (i_dmrs_1 < last_hop_symbol) {
    auto combine_pilots = [&, i_dmrs = 2](size_t i_symbol) mutable {
      ...
      srsvec::add(pilots_lse, pilot_products, pilots_lse);
          rx_sym =rx_pilots.get_symbol(i_dmrs, i_layer);
         tx_sym = pilots.get_symbol(hop_offset + i_dmrs, i_layer);
